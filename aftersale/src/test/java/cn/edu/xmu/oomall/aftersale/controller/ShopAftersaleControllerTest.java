...
class ShopAftersaleControllerTest {
    @Autowired
    private MockMvc mockMvc;
    @MockBean
    private RedisUtil redisUtil;

    private static final String APPROVE = "/aftersales/{id}/approve"; //put

    @Test
    void APPRROVE() throws Exception {
//        Mockito.when(redisUtil.hasKey(Mockito.anyString())).thenReturn(false);
//        Mockito.when(redisUtil.get(Mockito.anyString())).thenReturn(null);
//        Mockito.when(redisUtil.set(Mockito.anyString(), Mockito.any(), Mockito.anyLong())).thenReturn(true);
        String body="{\"result\": true,\"commit\": \"CommitStringHere\"}";
        this.mockMvc.perform(MockMvcRequestBuilders.put(APPROVE,1)
